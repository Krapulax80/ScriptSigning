...
#run this function on BNWITRDS only !
...
# Sign all scripts (with the first - position 0 - certificate
$Scripts = (Get-ChildItem -Path $ScriptFolder -Recurse | Where-Object {$_.Name -Like "*.ps1"}).FullName
    foreach ($script in $Scripts)
    {
    # Check, if the script is signed...
    $signed_status = $null
    $signed_status = (Get-AuthenticodeSignature -FilePath $script).Status
        # ...if not "valid", sign the script & update the status
        if ($signed_status -notmatch "Valid")
        {
        Write-Host "$script needs signing. Processing this..." -ForegroundColor Yellow
        [void] (Set-AuthenticodeSignature $script @(Get-ChildItem cert:\CurrentUser\My -codesign)[0])
            #Check again
            $signed_status = $null
            $signed_status = (Get-AuthenticodeSignature -FilePath $script).Status
            If ($signed_status -notmatch "Valid") {Write-Host "... $script is still [$signed_status]. Please review!" -ForegroundColor Red}
            else {Write-Host "... $script is now signed [$signed_status]." -ForegroundColor Green}
        }
    # Verify success - catch errors. (If script is still unsigned)
...
# eg:
# New-SignedScript -script "\\BNWINFRATS01.westcoast.co.uk\c$\Scripts\Generic\Lockouts\WestCoastAlertToTeams.ps1"
# SIG # Begin signature block
# MIIOWAYJKoZIhvcNAQcCoIIOSTCCDkUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVJHb917Zp00GsF9kBCfNrHzj
# 60WgggueMIIEnjCCA4agAwIBAgITTwAAAAb2JFytK6ojaAABAAAABjANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQU7DY6OnIK0EY1kpT/ayWcGnDMoZowDQYJKoZI
# hvcNAQEBBQAEggEA3jyzBJV6vy8PtlsatbtBHsUSLJFdiVblT0thS035FVJEDOLk
# 5b74cbQVPo/Xia7kVKgCG/jgI8eA4Pgg/7+fVrU1BH6zTlc9WZWmB1KH8EBBTAQD
# Sxm/0MNk+1v+xr9yxTur1dZ+WT0eujqY4hzUtt2e69o3TKCEYKRRmuPigybf/AcE
# FUIblZFiK+Hi0kU3uJz6L40hz6BauN5f49PX+yHonp3nWo5o9tLmyFU16O4nGOQV
# foll/wZwTimJhLDKmlKL3mGUFKQ9qH1r6sapVTF7JE+YtBrxzxaj6aE4gZhEj525
# D6XAXgmLxhIFj3pAEyv+P2pPetTahfmtS0rFGw==
